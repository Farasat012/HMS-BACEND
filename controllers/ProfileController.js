...
const updateAdmin = async (req, res) => {
    ...
    if (!userValidStatus.status) {
        ...
        try {
            ...
            const updateduser = await User.updateOne({ _id: req.params.id }, { $set: updateData });
...
const updateDoctor = async (req, res) => {
    ...
    if (!doctorValidStatus.status) {
        ...
        try {
            ...
            // Hash the password before updating
            const hashedPassword = await bcrypt.hash(req.body.password, 10);
            const updateduser = await User.updateOne({ _id: req.body.userId }, { $set: { "firstName": req.body.firstName, "lastName": req.body.lastName,"email":req.body.email, "username": req.body.username, "password": hashedPassword } });
